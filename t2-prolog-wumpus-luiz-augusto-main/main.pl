...
%pegar	
pegar :- posicao(X,Y,_), tile(X,Y,'O'), retract(tile(X,Y,'O')), assert(tile(X,Y,'')), atualiza_pontuacao(-5), atualiza_pontuacao(500), set_real(X,Y), ouro_pego(O), O1 is O + 1, retract(ouro_pego(O)), assert(ouro_pego(O1)),!. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
executa_acao(fim) :- posicao(1,1,_), ouro_pego(O), O == 3, !.
executa_acao(voltar) :- ouro_pego(O), O == 3, !.
executa_acao(pegar) :- posicao(PX,PY,_), memory(PX,PY,[brilho]), !.
executa_acao(pegar) :- posicao(PX,PY,_), memory(PX,PY,[reflexo]), energia(E), E =< 80, !.


executa_acao(andar) :- posicao(PX,PY,norte), PYY is PY + 1, memory(PX,PYY,[]), PY < 12, \+visitado(PX,PYY), !.
executa_acao(andar) :- posicao(PX,PY,leste), PXX is PX + 1, memory(PXX,PY,[]), PX < 12, \+visitado(PXX,PY), !.
executa_acao(andar) :- posicao(PX,PY,sul), PYY is PY - 1, memory(PX,PYY,[]), PY > 1, \+visitado(PX,PYY), !.
executa_acao(andar) :- posicao(PX,PY,oeste), PXX is PX - 1, memory(PXX,PY,[]), PX > 1, \+visitado(PXX,PY), !.


executa_acao(virar_direita) :- posicao(PX,PY,norte), PXX is PX + 1, memory(PXX,PY,[]), PX < 12, \+visitado(PXX,PY), !.
executa_acao(virar_direita) :- posicao(PX,PY,leste), PYY is PY - 1, memory(PX,PYY,[]), PY > 1, \+visitado(PX,PYY), !.
executa_acao(virar_direita) :- posicao(PX,PY,sul), PXX is PX - 1, memory(PXX,PY,[]), PX > 1, \+visitado(PXX,PY), !.
executa_acao(virar_direita) :- posicao(PX,PY,oeste), PYY is PY + 1, memory(PX,PYY,[]), PY < 12, \+visitado(PX,PYY), !.
executa_acao(virar_esquerda) :- posicao(PX,PY,norte), PXX is PX - 1, memory(PXX,PY,[]), PX > 1, \+visitado(PXX,PY), !.
executa_acao(virar_esquerda) :- posicao(PX,PY,leste), PYY is PY + 1, memory(PX,PYY,[]), PY < 12, \+visitado(PX,PYY), !.
executa_acao(virar_esquerda) :- posicao(PX,PY,sul), PXX is PX + 1, memory(PXX,PY,[]), PX < 12, \+visitado(PXX,PY), !.
executa_acao(virar_esquerda) :- posicao(PX,PY,oeste), PYY is PY - 1, memory(PX,PYY,[]), PY > 1, \+visitado(PX,PYY), !.


executa_acao(powerup) :- energia(E), E =< 80, memory(_,_,[reflexo]), !.
